...
from reflect.data.noise import NormalNoise, EnvNoise, LinearSegmentNoiseND
...
@pytest.mark.parametrize("noise_generator", [
    NormalNoise(dim=8, noise_size=0.3),
    LinearSegmentNoiseND(dim=8, noise_size=0.3)
])
def test_data_loader_noise_generator(noise_generator, world_model_actor):
    num_time_steps = 18
