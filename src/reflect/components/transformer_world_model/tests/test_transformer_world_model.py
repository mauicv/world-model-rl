...
def test_world_model(transformer_world_model: TransformerWorldModel):
    ...
    assert next_seq.state_dist.base_dist.probs.shape == (2, 16, 8, 8)
    assert next_seq.state_sample.shape == (2, 16, 64)
    assert next_seq.reward.base_dist.mean.shape == (2, 16, 1)
    assert next_seq.done.base_dist.mean.shape == (2, 16, 1)
    assert first_seq.state_dist.base_dist.logits.shape == (2, 16, 8, 8)
    assert first_seq.state_sample.shape == (2, 16, 64)
    assert first_seq.action.shape == (2, 16, 1)
...
def test_imagine_rollout(transformer_world_model: TransformerWorldModel, actor: Actor):
    ...
    assert imagined_rollout.state.shape == (32, 11, 64)
    assert imagined_rollout.reward.shape == (32, 11, 1)
    assert imagined_rollout.done.shape == (32, 11, 1)
    assert imagined_rollout.observations.shape == (32, 11, 3, 64, 64)
    assert imagined_rollout.action.shape == (32, 11, 1)
