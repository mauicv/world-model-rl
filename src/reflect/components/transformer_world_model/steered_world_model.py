...
class SteeredWorldModel(Base):
    ...
    def update(
        ...
        r_pred = r_pred[:, -1]
        steer_reward_loss = steer_loss_fn(r_targets, r_pred)
        # Update observation_model and dynamic_model
        dyn_loss = (
            params.dynamic_coeff * dynamic_loss 
            + params.reward_coeff * steer_reward_loss
        ...
        losses = WorldModelLosses(
            ...
            reward_loss= steer_reward_loss.detach().cpu().item(),
        ...
        with FreezeParameters([self.dynamic_model, self.decoder]):
            ...
            for i in range(num_timesteps):
                new_z, new_r, new_d = self \
                    .dynamic_model.rstep(z=z, a=a, d=d)
                if with_entropies:
                    action_dist = actor(
                        new_z[:, -1, :].detach(),
                        deterministic=False
                ...
                z, a, d = new_z, new_a, new_d
            to_return = [z, a, new_r[:, -1], d]
            if with_entropies:
